 ...
 * - 导入与所有编辑成功后的工作集始终遵守**同一组聚合约束**（条目数
 *   1..50000、总长 ≤ 300000，单条 1..200、字符集、互不重复；由 masker
 *   的编辑函数在重算前校验）。越界增改或低于最小数量的删除抛
 *   INVALID_PATTERN、在重算之前被拒绝，绝不触发自动机构建；因此任何已
 *   提交快照（含采纳稿）都满足原输入契约，可再次被 parseInput 载入。
 ...
 * mutate 抛 INVALID_PATTERN（非法值/重复/聚合越界，如 50001 条、
 * 总长超 300000、删除唯一条目）时原样向上抛；recompute 抛 COUNT_FAILED
 * 时同样向上抛——两种情况下调用方都保留旧快照不变。聚合违规发生在
 * 重算之前，因此不会为越界候选重建自动机。
 */
